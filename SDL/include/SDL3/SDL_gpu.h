 ...
 *
 * You must sample input AFTER acquiring or waiting for a swapchain texture,
 * or else the input won't be fresh!
 *
 * \param command_buffer a command buffer.
 * \param window a window that has been claimed.
 * \param swapchain_texture a pointer filled in with a swapchain texture
...
/**
 * Waits for the next available vblank
 * You should call this at the start of your while loop before sampling input
 * If you do not do this, your while loop will quickly enqueue four frames to
 * the CAMetalLayer and there'll be four frames of lag to everything
 *
 * \param window a window that has been claimed.
 * \since This function is available since SDL 3.4.0.
 */
extern SDL_DECLSPEC void SDL_OSXWaitForDisplayLink(SDL_Window* window);
